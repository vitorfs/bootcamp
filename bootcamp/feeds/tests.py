...
class FeedsTest(TestCase):
    ...
    def setUp(self):
        ...
        'Setup feeds'
        Feed.objects.create(post="My first post", likes=3, user=self.user1)
        Feed.objects.create(post="Commenting feed1", likes=2, user=self.user2)
        Feed.objects.create(post="Second comment in feed1", likes=4, user=self.user1)
